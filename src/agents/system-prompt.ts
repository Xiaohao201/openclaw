...
function buildSkillsSection(params: { skillsPrompt?: string; readToolName: string }) {
  ...
  return [
    ...
    `- Single task (one skill applies, or several overlap): pick the most specific match, read its SKILL.md at <location> with \`${params.readToolName}\`, then follow it.`,
    ...
    "Constraints: read a SKILL.md only after selecting it, and only for a skill you will actually act on; do not preload skills you won't use.",
...
export function buildAgentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "Runtime channel_capabilities describes messaging-channel features, not tool availability. A value of none does not disable tools; use the actual tool definitions for this run.",
    toolLines.length > 0
      ? toolLines.join("\n")
      : params.toolNames !== undefined
        ? "No tools are available in this run. Do not claim that this means the deployment cannot support tools."
        : [
