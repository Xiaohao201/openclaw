...
describe("factory gating", () => {
  ...
  it("hides tools from non-rabbitmq agents and exposes to chat agents", () => {
    expect(
      createOpinionContentToolFactory(fakeApi, resolver, s)({ agentId: "telegram-1" }),
    ).toBeNull();
    expect(
      createOpinionContentToolFactory(fakeApi, resolver, s)({ agentId: "rabbitmq-1749" })?.name,
    ).toBe("opinion_content_create");
    expect(
      createReportStopToolFactory(fakeApi, resolver, s)({ agentId: "rabbitmq-1749" })?.name,
    ).toBe("report_stop");
  });
});
...
describe("opinion_content_create", () => {
  it("requires target+instruction for Respond mode", async () => {
    const tool = createOpinionContentToolFactory(
      fakeApi,
      resolver,
      store(),
    )({ agentId: "rabbitmq-1749" })!;
    const res = parse(
      await tool.execute("o1", { category: "Respond", content: "事件", data: "正面" }),
    );
...
describe("report_status", () => {
  it("polls fetch-list by stored slug and category", async () => {
    const s = store();
    const create = createOpinionContentToolFactory(
      fakeApi,
      resolver,
      s,
    )({ agentId: "rabbitmq-1749" })!;
    const status = createReportStatusToolFactory(
      fakeApi,
      resolver,
      s,
    )({ agentId: "rabbitmq-1749" })!;
    mockPostForm.mockResolvedValue({ code: "success", reportId: "SLUGX" });
    await create.execute("c1", { category: "Comment", content: "舆情简报", data: "正面阐述" });
    ...
    expect(query).toMatchObject({ category: "Comment" });
  ...
  it("resolves the numeric id via fetch-list then calls stop-report", async () => {
    ...
    mockGetJson
      .mockResolvedValueOnce({
        code: "success",
        list: [{ id: 99, slug: "STOPME", status: "Running" }],
      })
