...
import { isQwenCodingPlanBaseUrl, isQwenStandardOnlyModel, QWEN_BASE_URL } from "./models.js";
import {
  applyQwenConfig,
  applyQwenConfigCn,
...
function isQwenStandardOnlyModelUnsupportedForConfig(params: {
...
export default defineSingleProviderPluginEntry({
  ...
  provider: {
    ...
    auth: [
      {
        ...
        noteMessage: [
          ...
          "Models: qwen3.8-flash, qwen3.6-plus, qwen3.5-plus, qwen3-coder-plus, etc.",
        ].join("\n"),
    ...
    normalizeConfig: ({ providerConfig }) => {
      ...
      const models = providerConfig.models?.filter((model) => !isQwenStandardOnlyModel(model.id));
    ...
    suppressBuiltInModel: (ctx) => {
      ...
      if (
        ...
        !isQwenStandardOnlyModel(ctx.modelId) ||
        !isQwenStandardOnlyModelUnsupportedForConfig({ config: ctx.config, baseUrl: ctx.baseUrl })
      ) {
        return undefined;
      }
      return {
        suppress: true,
        errorMessage: `Unknown model: qwen/${ctx.modelId}. ${ctx.modelId} is not supported on the Qwen Coding Plan endpoint; use a Standard pay-as-you-go Qwen endpoint or choose qwen/qwen3.5-plus.`,
      };
    },
  },
  register(api) {
