  ...
  return {
    description:
      braveMode === "llm-context"
        ...
        : "Search the web using Brave Search API. Supports region-specific and localized search via country and language parameters. Returns titles, URLs, and snippets for fast research. For breaking or time-sensitive claims, first search with freshness=day and count=10, then run a separate unfiltered authority search; use web_fetch to open and verify the newest critical sources before concluding. If a named event returns absent or weak results, retry with shorter entity and distinctive-action variants plus relevant platform/domain queries. Search-index absence must not be treated as proof that a user-reported event did not occur.",
